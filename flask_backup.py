...
#question rewriter
websystem = """You a question re-writer that converts an input question to a better version that is optimized \n
     for web search. Look at the input and try to reason about the underlying semantic intent / meaning. Also add SanaExpert to the Question if it is not stated as all the questions will be about SanaExpert."""
...
def chat():
        ...
        if "messages" in event:
            ...
            for message in event["messages"]:
                ...
                if hasattr(message, "content") and "AIMessage" in str(type(message)):
                   ...
                   # print("Assistant response:", message.tool_calls[0]["name"])
                    

                    # Determine the source of the response
                    if hasattr(message, "tool_calls") and message.tool_calls:
                        # Extract the tool name from the first tool call
                        sources.append(message.tool_calls[0]["name"])
                        source = message.tool_calls[0]["name"]
                    elif hasattr(message, "tool_call_id"):
                        # If the message is a ToolMessage, set the source to the tool name
                        source = "tool"
                    else:
                        # Default to "assistant" if no tool was used
                        source = "assistant"
    ...
    return jsonify({
        ...
        "source": sources,  # Include the source in the response
